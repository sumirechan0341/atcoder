...
## 思考
N x Nのマス目と整数Mが与えられる。
(0, 0)からスタートして、ちょうど距離が\(\sqrt{M}\)のマス目にのみ移動できるとき、各マス目に対する最小移動回数を求める問題。

Mが与えられたときに、どんな移動ができるかをまず考える。Mを2つ平方数に分割できるとき、移動できるマス目が存在するので、そのようなペアをすべて探す。
ペアとしてありうる最小値の0をleftに、ありうる最大値の1000をrightに設定して、leftを1ずつ増やしながら、rightの範囲を削っていく方法でペアをすべて求めた。

この後が問題である。数学パワーで移動量の倍数の処理でいけないかなーとか考えてみるが、まあ無理である。
となるとできることは、全探索である。
最短距離を求めたいのでBFSですべてのマスの最短経路を求めていく。
現在の座標から、先に求めたありうる移動量をすべて試せばよい。
